...
$connect = new PDO("mysql:host=localhost; dbname=post", "root", "ehdgus48350");
...
$query = "
SELECT * FROM post.webslesson_post
";
...
$query .= 'ORDER BY tbl_webslesson_post_id ASC ';
...
if($total_data > 0)
{
	foreach($result as $row)
	{
		$output .= '
    <tr>
      <td >'.$row["tbl_webslesson_post_id"].'</td>
...
if($total_links > 9)
{
	...
	else
	{
		...
		else
		{
			$page_array[] = 1;
			$page_array[] = '...';
			for($count = $page - 4; $count <= $page + 4; $count++)
			{
				$page_array[] = $count;
			}
			$page_array[] = '...';
